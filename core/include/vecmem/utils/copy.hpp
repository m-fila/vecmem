...
/// event
struct no_event_t {
    explicit no_event_t() = default;
};
/// Constant object of type @c no_event_t to use as an argument for functions
/// that do not need to return an event
inline constexpr no_event_t no_event{};
...
public:
    ...
    template <typename TYPE>
    void setup(no_event_t, data::vector_view<TYPE> data) const;
    ...
    template <typename TYPE>
    void memset(no_event_t, data::vector_view<TYPE> data, int value) const;
    ...
    /// Copy a 1-dimensional vector's data without returning an event
    template <typename TYPE>
    void operator()(no_event_t,
    ...
    /// Set up the internal state of a jagged vector buffer correctly on a
    /// device without returning an event
    template <typename TYPE>
    void setup(no_event_t, data::jagged_vector_view<TYPE> data) const;
    ...
    /// event
    template <typename TYPE>
    void memset(no_event_t, data::jagged_vector_view<TYPE> data,
                int value) const;
    ...
    /// Copy a jagged vector's data without returning an event
    template <typename TYPE>
    void operator()(
        no_event_t,
    ...
    /// Set the sizes of a resizable jagged vector without returning an event
    template <typename TYPE>
    void set_sizes(
        no_event_t,
        const std::vector<typename data::vector_view<TYPE>::size_type>& sizes,
    ...
    /// returning an event
    template <typename SCHEMA>
    void setup(no_event_t, edm::view<SCHEMA> data) const;
    ...
    /// event
    template <typename... VARTYPES>
    void memset(no_event_t, edm::view<edm::schema<VARTYPES...>> data,
                int value) const;
